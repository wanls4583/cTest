...
unsigned char * set_data(unsigned char *target, unsigned char *str) {
  int length;
  unsigned char *data;
...
{ 
  ...
  // printf("send:%d\n", send_buffer_size);
  // show_hex(send_buffer, send_buffer_size);
                                   ...
                                   TLSParameters *parameters )
{
  ...
  show_hex(parameters->master_secret, 48);
}
                                 ...
                                 TLSParameters *parameters )
{
  ...
  show_hex(parameters->client_random, 32);
...
{ 
  ...
  show_hex(parameters->server_random, 32);
                                                 ...
                                                 TLSParameters *parameters )
{
  ...
  // printf("rsa_key:\n");
  // show_hex(private_key.modulus->rep, private_key.modulus->size);
  ...
  show_hex(premaster_secret, premaster_secret_length);
                            ...
                            TLSParameters *parameters )
{
  ...
  else
  {
    ...
    // show_hex(encrypted_message, message.length);

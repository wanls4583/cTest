...
void test_aes_128_ccm() {
    ...
    aes_ccm_encrypt(input, inputLen, add, addLen, ciphertext, iv, key);
    show_hex(ciphertext, inputLen + MAC_LENGTH);
    aes_ccm_decrypt(ciphertext, inputLen + MAC_LENGTH, add, addLen, out, iv, key);
    printf("%s\n", out);
}
...
void test_aes_128_gcm() {
    ...
    aes_gcm_encrypt(input, input_len, add, addLen, ciphertext, iv, key);
    show_hex(ciphertext, input_len + AES_BLOCK_SIZE);
    aes_gcm_decrypt(ciphertext, input_len + AES_BLOCK_SIZE, add, addLen, out, iv, key);
    printf("%s\n", out);
}
